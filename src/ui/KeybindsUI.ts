...
export class KeybindsUI {
  ...
  // Bottom edge of the panel in its current (collapsed/expanded) state.
  get bottom(): number {
    return PANEL_Y + HEADER_H + (this.collapsed ? 0 : this.bodyHeight());
  }

  // Right edge and top edge — fixed regardless of collapse state, used to
  // anchor EventLogUI beside this panel (same row) instead of stacking below
  // it (an open InventoryMenu panel used to cover the log there).
  get right(): number {
