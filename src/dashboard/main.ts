...
const ENEMIES: EnemyStat[] = [
  ...
  },
  {
    ...
    aggro: 150,
    attacks: [
      { label: "Ambush lunge (locked line, 215px, +150 kb, +BLEED 7/s×6s)", damage: 85, telegraphMs: 430 },
      { label: "Chomp (40px reach, +80 kb, +BLEED 5/s×4s)", damage: 62, telegraphMs: 460 },
    ],
    loot: "1-2 Mirehide + 1-2 Mirejaw Meat",
    trophy: "Mirejaw Trophy (elite)",
    notes: "Biome 3 SIGNATURE AMBUSHER and the game's ONLY Mirehide source (locked) — the bayou light-armor reforge tier is gated behind hunting it. Lurks half-sunk (alpha 0.4 — VISIBLE, unlike the Sandmaw's 0.18) and creeps into position, then commits a LOCKED-LINE lunge chomp you sidestep. Unlike the Sandmaw it does NOT re-submerge after one attempt: it surfaces and HUNTS, chomping in melee and re-lunging from mid-range, and only re-buries once it loses you. Resists pierce ×0.5 (bony scutes), weak slash ×1.25 (soft belly) — the inverse of the Fenlurker.",
  },
  {
  ...
  },
  {
    ...
    speed: 36,
    aggro: 165,
    attacks: [{ label: "Overhead smash (62px reach, +240 knockback)", damage: 95, telegraphMs: 780 }],
  ...
  },
  {
    ...
    speed: 104,
    aggro: 190,
    attacks: [{ label: "Claw (skitter-slash, shortest telegraph in the game)", damage: 50, telegraphMs: 150 }],
    loot: "1-2 Gloam Dust",
  ...
  },
  {
    ...
    speed: 40,
    aggro: 260,
    attacks: [{ label: "Gloam orb (SLOW HOMING projectile, magic — BYPASSES armor)", damage: 26 }],
    loot: "3-5 Hex Essence",
    trophy: "Corpselight Trophy (elite)",
    notes: "Biome 3's ONE ranged creature and deliberately UNCOMMON (~1/3 of any melee species) — the exception that keeps the roster reading melee-core. Fires the game's FIRST HOMING projectile (Projectile.homing): a bounded reversal of the anti-kite governor — 110px/s (under a walking player), 1.5 rad/s turn rate (lateral movement out-turns it into a lazy overshoot), 4.2s hard lifetime (an overshot orb expires instead of orbiting forever). STANDING STILL is what gets you hit. Magic damage bypasses flat armor, so bayou plate is no answer — footwork is. Holds a ~210px standoff and drifts, but has NO blink/escape, so closing the gap really does beat it. Neutral to physical on purpose (the Hexling's old flat physical resist felt unkillable); weak fire ×1.25 + magic ×1.25. Also the bayou's local HEX ESSENCE source, so forging Gloamsteel doesn't require walking back to the badlands.",
  },
  {
