...
const ENEMIES: EnemyStat[] = [
  ...
  },
  {
    ...
    aggro: 250,
    attacks: [
      { label: "Overhead smash (88px reach, +300 knockback, +ENFEEBLE -30% dmg dealt × 10s)", damage: 98, telegraphMs: 780 },
    ...
    notes: "Biome 3 BRUISER (the Cragscale analog) — the slowest common enemy and the tankiest. C2 (2026-07-23): fixed the user's 'lacks attack moves / feels weird' — it can't catch you so it now STOPS you (a mid-range SPORE BURST that drops a slowing/poisoning cloud on your retreat path, setting up the smash), and on death it comes apart into 3 MOSSLINGS (scale 0.58, 16% HP, forceAggro'd — killing one in a bad spot swarms you; spawnlings never chain-spawn and drop no trophy). ONE main attack with the longest common-roster wind-up (780ms) + 720ms recovery, so baiting the smash is the intended fight. 2026-07-24: damage-type resistances REMOVED game-wide (the user); smash 78→98 (still the biggest COMMON hit, ~half of Sanguinarch, elite ×1.5=147 — the user's '192 from elite tree guy' is fixed). BAYOU DEBUFF SYSTEM (2026-07-26): the smash now ENFEEBLES (-30% damage dealt, 10s — the longest debuff in the game, since it never takes control away and can only be felt by outlasting several swing cycles) — the roster's teacher for it. On the SMASH rather than the spore cloud on purpose: the cloud routes through environmentEffectAt, which makes it terrain, and terrain is deliberately undispellable. So the counterplay is the one the creature was already built around — bait the 780ms wind-up.",
  },
  {
    name: "Murkling (BAYOU)",
    hp: 40,
  ...
  },
  {
    ...
    attacks: [
      ...
      { label: "Collapse slam (magic AoE 104px on transform-IN, +150 kb, +SILENCE 6s — dodge the tell)", damage: 26, telegraphMs: 560 },
  ...
  },
  {
    ...
    attacks: [
      ...
      { label: "Gorge Heave (190px radial SPACING shove, +620 kb, +DISARM 6s — it throws you out of melee, then you cannot swing on the way back in)", damage: 96, telegraphMs: 660 },
